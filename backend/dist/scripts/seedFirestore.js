...
const seedPersonalSchedules = async () => {
    ...
    personalSchedules.forEach((schedule) => {
        const docRef = firebase_1.db.collection('PersonalSchedule').doc();
...
const seedDepartmentSchedules = async () => {
    ...
    departmentSchedules.forEach((schedule) => {
        const docRef = firebase_1.db.collection('DepartmentSchedule').doc();
        batch.set(docRef, schedule);
    });
    await batch.commit();
...
const seedProjectSchedules = async () => {
    ...
    projectSchedules.forEach((schedule) => {
        const docRef = firebase_1.db.collection('ProjectSchedule').doc();
        batch.set(docRef, schedule);
    });
    await batch.commit();
...
const seedCompanySchedules = async () => {
    ...
    companySchedules.forEach((schedule) => {
        const docRef = firebase_1.db.collection('CompanySchedule').doc();
        batch.set(docRef, schedule);
    });
    await batch.commit();
...
const seedScheduleConflicts = async () => {
    ...
    scheduleConflicts.forEach((conflict) => {
        const docRef = firebase_1.db.collection('ScheduleConflict').doc();
...
const seedPersonalScheduleAnalyses = async () => {
    ...
    personalScheduleAnalyses.forEach((analysis) => {
        const docRef = firebase_1.db.collection('PersonalScheduleAnalysis').doc(analysis.date.toISOString());
...
const seedDepartmentScheduleAnalyses = async () => {
    ...
    departmentScheduleAnalyses.forEach((analysis) => {
        const docRef = firebase_1.db.collection('DepartmentScheduleAnalysis').doc(`${analysis.department_name}_${analysis.date.toISOString()}`);
        batch.set(docRef, analysis);
    });
    await batch.commit();
...
const seedProjectScheduleAnalyses = async () => {
    ...
    projectScheduleAnalyses.forEach((analysis) => {
        const docRef = firebase_1.db.collection('ProjectScheduleAnalysis').doc(`${analysis.project_id}_${analysis.date.toISOString()}`);
        batch.set(docRef, analysis);
    });
    await batch.commit();
...
const seedCompanyScheduleAnalyses = async () => {
    ...
    companyScheduleAnalyses.forEach((analysis) => {
        const docRef = firebase_1.db.collection('CompanyScheduleAnalysis').doc(analysis.schedule_id);
        batch.set(docRef, analysis);
    });
    await batch.commit();
...
const seedComprehensiveAnalysisReports = async () => {
    ...
    comprehensiveAnalysisReports.forEach((report) => {
        const docRef = firebase_1.db.collection('ComprehensiveAnalysisReport').doc();
...
const seedAIConflictScheduleAnalyses = async () => {
    ...
    aiConflictScheduleAnalyses.forEach((analysis) => {
        const docRef = firebase_1.db.collection('AIConflictScheduleAnalysis').doc();
        batch.set(docRef, analysis);
    });
    await batch.commit();
